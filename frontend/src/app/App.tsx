          ...
          <ResponsiveContainer width="100%" height={175}>
            <BarChart data={PEAK_DATA} barSize={13} margin={{ top: 4, right: 4, bottom: 0, left: -18 }}>
              ...
              <Bar dataKey="v" radius={[4, 4, 0, 0]} name="Accidents">
                {PEAK_DATA.map((d, i) => (
                  <Cell key={i} fill={d.v > 20 ? "#ef4444" : d.v > 12 ? "#f59e0b" : "#3b82f6"} fillOpacity={0.85} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
...
export default function App() {
  ...
  const cards = [
    ...
    { id: "dl",  title: "Deep Learning",        Icon: Eye,          accent: "#6366f1", content: <DeepLearningCard /> },
    { id: "ai",  title: "GenAI Assistant",      Icon: MessageSquare, accent: "#06b6d4", content: <GenAIChatCard /> },
      ...
      <div className="relative z-10 max-w-[1640px] mx-auto px-4 sm:px-6 py-4">
        ...
        <header className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Voice */}
            <button
              onClick={() => { setVoice(v => !v); toast(voice ? "Voice deactivated" : "Voice assistant activated — listening…") }}
              className="p-2 rounded-xl transition-all"
              style={{
                background: voice ? "rgba(59,130,246,0.18)" : "var(--glass-bg)",
                border: `1px solid ${voice ? "rgba(59,130,246,0.35)" : "var(--glass-border)"}`,
                color: voice ? "#60a5fa" : "var(--txt-muted)",
              }}>
            ...
            <button onClick={() => setNotif(true)}
              ...
              <span className="absolute top-1 right-1 w-2 h-2 bg-red-400 rounded-full ring-2 ring-current" style={{ ringColor: "var(--glass-bg)" }} />
            </button>

            {/* SOS */}
            <button
              onClick={() => toast.error("🆘 SOS activated — Emergency services notified!", { duration: 5000 })}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-red-400 transition-all hover:bg-red-400/20 active:scale-95"
        ...
        <Glass className="mb-5 p-5 float-card" style={{ animationDuration: "9s" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-4">
            ...
            {/* Date & Time */}
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: "var(--txt-muted)" }}>Date & Time</label>
              <div className="relative">
                <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-sky-400" />
                <input type="datetime-local"
                  defaultValue={new Date(Date.now() + 15 * 60000).toISOString().slice(0, 16)}
                  className="input-field" />
              </div>
            </div>
